...
class request_performer(Thread):
    def __init__(self, passwd, url, user, method):
...
def start(argv):
    ...
    if len(sys.argv) < 5:
        ...
        sys.exit()
    try:
        opts, args = getopt.getopt(argv, "w:u:t:f:m:")
    ...
    launcher_thread(passwds, threads, url, username, meth)


def launcher_thread(passwd, th, url, username, met):
    global i
    i = []
    i.append(0)
        ...
        if hit == "1":
            try:
                if i[0] < th:
                    n = passwd.pop(0)
                    i[0] = i[0] + 1
                    thread = request_performer(n, url, username, met)
                    thread.start()
